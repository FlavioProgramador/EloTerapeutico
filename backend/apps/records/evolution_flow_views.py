"""Endpoints do modal de criação e edição de evoluções clínicas."""
...
class PatientEvolutionFlowView(ClinicalPatientMixin, APIView):
    """Lista e cria evoluções no contexto imutável do paciente da URL."""
...
class EvolutionFlowDetailView(ClinicalPatientMixin, APIView):
    """Consulta, edita parcialmente e arquiva uma evolução."""
    ...
    def patch(self, request, pk):
        evolution = self.get_evolution(pk)
        previous_confidential = evolution.is_confidential
        serializer = EvolutionFlowSerializer(
            evolution,
            data=request.data,
            partial=True,
        ...
        serializer.is_valid(raise_exception=True)
        evolution = serializer.save()
        confidentiality_changed = previous_confidential != evolution.is_confidential
        log_access(
            request,
            ...
            obj_repr=(
                ...
                f"confidencialidade_alterada={confidentiality_changed}; "
                f"confidencial={evolution.is_confidential}"
            ),
        )
        return Response(
    ...
    def delete(self, request, pk):
        ...
        if evolution.created_by_id != request.user.id and not request.user.is_admin_role:
            self.permission_denied(
                request,
                message="Somente o autor ou administrador autorizado pode arquivar.",
            )
        profile, _ = EvolutionClinicalData.objects.get_or_create(
            evolution=evolution,
            defaults={"updated_by": request.user},
        )
        if profile.status != EvolutionClinicalData.Status.ARCHIVED:
...
class PatientEvolutionAppointmentOptionsView(ClinicalPatientMixin, APIView):
    """Consultas do paciente disponíveis para vínculo, sem confiar em IDs do cliente."""
...
class EvolutionAttachmentListCreateView(ClinicalPatientMixin, APIView):
    ...
    def post(self, request, evolution_id):
        ...
        if (
            ...
            self.permission_denied(
                request,
                message="Anexos só podem ser incluídos pelo autor enquanto a evolução estiver editável.",
            )
        current_count = evolution.documents.filter(
            deleted_at__isnull=True,
            is_archived=False,
        ).count()
        if current_count >= max_evolution_attachments():
            return Response(
                {"file": ["O limite de anexos desta evolução foi atingido."]},
...
class EvolutionAttachmentDetailView(ClinicalPatientMixin, APIView):
    ...
    def delete(self, request, evolution_id, attachment_id):
        ...
        if (
            ...
            self.permission_denied(
                request,
                message="Este anexo não pode mais ser removido.",
            )
...
class EvolutionAttachmentDownloadView(EvolutionAttachmentDetailView):
    def get(self, request, evolution_id, attachment_id):
        ...
        if not can_view_confidential_evolution(request.user, evolution):
            self.permission_denied(
                request,
                message="Você não pode acessar anexos desta evolução confidencial.",
            )
        try:
...
class ClinicalEvolutionTemplateListCreateView(APIView):
    ...
    def _ensure_clinical_user(self, request):
        if request.user.is_secretary:
            self.permission_denied(
                request,
                message="Secretárias não podem acessar templates clínicos.",
            )
    ...
    def get(self, request):
        ...
        return Response(
            ClinicalEvolutionTemplateSerializer(queryset, many=True).data
        )
...
class ClinicalEvolutionTemplateDetailView(APIView):
    ...
    def get_object(self, request, pk):
        template = get_object_or_404(
            ClinicalEvolutionTemplate.objects.select_related("owner"), pk=pk
        )
        if request.user.is_secretary:
        ...
        if template.owner_id is None and not request.user.has_perm(
            ...
            self.permission_denied(
                request,
                message="Somente administradores autorizados alteram templates globais.",
            )
        return template
