...
from django.db import models, transaction, connection, NotSupportedError, OperationalError
...
class Command(BaseCommand):
    ...
    def handle_shutdown(self, signum, frame):
        self.stdout.write(self.style.WARNING("Sinal de parada recebido. Encerrando worker de forma graciosa após o término do job atual..."))
    ...
    def claim_next_job(self):
        ...
        with transaction.atomic():
            queryset = ClinicalExport.objects.filter(
    ...
    def process_job(self, job):
        ...
        try:
            ...
            job.error_message = ""
            job.save()
            self.stdout.write(self.style.SUCCESS(f"Job #{job.id} concluído com sucesso. Tamanho: {job.size_bytes} bytes."))
