...
def test_linked_therapist_cannot_modify_goal_of_another_author(security_context):
    ...
    response = client.patch(f"/api/v1/records/goals/{goal.id}/", {"title": "Modified by Linked"}, format="json")
...
def test_owner_can_modify_own_goal(security_context):
    ...
    response = client.patch(f"/api/v1/records/goals/{goal.id}/", {"title": "New Title"}, format="json")
