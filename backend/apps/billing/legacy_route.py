...
def legacy_billing_route_enabled() -> bool:
    ...
    A configuração Django tem precedência. A variável de ambiente existe para
    permitir desligar a rota sem adicionar um novo contrato público ao módulo
    de settings durante a janela de depreciação.
    """
    ...
    raw_value = os.getenv("BILLING_LEGACY_ROUTE_ENABLED", "true").strip().lower()
