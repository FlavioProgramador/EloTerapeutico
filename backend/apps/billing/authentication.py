...
class SubscriptionRequired(APIException):
    status_code = 402
    default_detail = "É necessário possuir uma assinatura ativa ou um teste gratuito válido."
...
class SubscriptionJWTAuthentication(JWTAuthentication):
    ...
    def authenticate(self, request):
        ...
        if not entitlement.allowed:
            raise SubscriptionRequired(code=entitlement.code, message=entitlement.message)
        return user, token
