...
class AppointmentRecurrenceSerializer(serializers.ModelSerializer):
    ...
    def get_completed_count(self, obj):
        if hasattr(obj, "_prefetched_objects_cache") and "appointments" in obj._prefetched_objects_cache:
            return sum(1 for a in obj.appointments.all() if a.status == Appointment.Status.COMPLETED)
    ...
    def _next(self, obj):
        if hasattr(obj, "_prefetched_objects_cache") and "appointments" in obj._prefetched_objects_cache:
            ...
            upcoming = [
                ...
                if a.start_time >= now and a.status in (Appointment.Status.SCHEDULED, Appointment.Status.CONFIRMED)
            ]
