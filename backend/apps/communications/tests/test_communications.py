...
@pytest.mark.django_db
@override_settings(BILLING_REQUIRE_SUBSCRIPTION=False)
def test_communications_list_query_count(
    authenticated_client,
    therapist,
    patient,
    ...
    for i in range(5):
        comm = create_communication(
            ...
            patient=patient,
            channel=Communication.Channel.EMAIL,
            category=Communication.Category.OTHER,
            subject=f"Assunto {i}",
            body=f"Conteúdo {i}",
    ...
    first_id = response.data["results"][0]["public_id"]

    detail_response = authenticated_client.get(f"/api/v1/communications/{first_id}/")
    assert detail_response.status_code == 200
    assert "attempts" in detail_response.data
    assert len(detail_response.data["attempts"]) == 1
