...
DEFAULT_AUTOMATION_BLUEPRINTS = [
    (
        ...
        "appointment-created",
        0,
        CommunicationAutomation.DelayUnit.MINUTES,
        False,
    ),
    (
        ...
        24,
        CommunicationAutomation.DelayUnit.HOURS,
        True,
    ),
    (
...
def _respect_delivery_window(
    value,
    automation: CommunicationAutomation,
    preference: CommunicationPreference | None,
):
...
def _automation_context(
    ...
    if patient is not None:
        context.update(
            {
                "patient_has_email": bool(patient.email),
                "patient_has_whatsapp": bool(patient.whatsapp or patient.phone),
        ...
        context.update(
            {
                ...
                "appointment_modality": appointment.modality,
            }
        )
    return context


def _resolve_event_organization(
    *,
    owner,
    organization=None,
    patient=None,
    ...
    if resolved:
        first = resolved[0]
        if any(item.pk != first.pk for item in resolved[1:]):
            raise ValidationError("O evento combina recursos de organizações diferentes.")
        return first
    ...
    if membership is None:
        raise ValidationError("Não foi possível determinar a organização do evento.")
...
def emit_domain_event(
    *,
    ...
    organization = _resolve_event_organization(
        owner=owner,
    ...
    for automation in active_automations_for_event(
        owner,
        event_type,
        organization=organization,
    ):
        ...
        try:
            ...
            adjusted_at = _respect_delivery_window(
                scheduled_at,
                automation,
                preference,
            )
            ...
            communication = create_communication(
                organization=organization,
                owner=owner,
                created_by=automation.created_by or owner,
            ...
            logger.warning(
                "communication_automation_failed",
                extra={
...
def cancel_pending_for_source(
    *,
    ...
    queryset = Communication.objects.filter(
        source_event__startswith=source_event_prefix,
        source_object_type=source_object_type,
        source_object_id=source_object_id,
        status__in=[
...
def ensure_default_automations(owner, *, organization=None) -> None:
    ...
    for (
        ...
        delay_unit,
        send_before,
    ) in DEFAULT_AUTOMATION_BLUEPRINTS:
        ...
        if template is None:
            continue
        CommunicationAutomation.objects.get_or_create(
            organization=organization,
            name=name,
            defaults={
                "owner": owner,
