...
from apps.forms.selectors import filtered_forms_for_owner
...
class FormListCreateView(UserFormMixin, APIView):
    ...
    def get(self, request):
        queryset = filtered_forms_for_owner(owner=request.user, params=request.query_params)
...
class FormArchiveView(UserFormMixin, APIView):
    def post(self, request, pk):
        form = archive_form(actor=request.user, form=self.get_form(pk))
        return Response(TherapeuticFormSerializer(form).data)


class FormRestoreView(UserFormMixin, APIView):
    def post(self, request, pk):
        form = restore_form(actor=request.user, form=self.get_form(pk))
        return Response(TherapeuticFormSerializer(form).data)


class FormDuplicateView(UserFormMixin, APIView):
    def post(self, request, pk):
        copy = duplicate_form(actor=request.user, source=self.get_form(pk))
