...
def _application_error_response(exc: ApplicationError, context, request_id: str) -> Response:
    ...
    public_message = exc.default_detail if exc.status_code >= 500 else exc.detail
    response = Response(
        {
            "error": {
                "code": exc.code.upper(),
...
def custom_exception_handler(exc, context):
    ...
    if isinstance(exc, ApplicationError):
        return _application_error_response(exc, context, request_id)
    ...
    # Não registre a representação textual da exceção: ela pode conter SQL,
    # tokens, CPF ou conteúdo clínico. Tipo, view e request_id são suficientes.
    logger.error(
