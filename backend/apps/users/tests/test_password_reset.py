...
class TestPasswordResetAPI:
    ...
    def test_confirm_reset_invalid_token(self, api_client, therapist_user):
        ...
        assert response.data["error"]["details"]["token"] == ["O link de redefinição é inválido ou expirou."]

    def test_confirm_reset_mismatched_passwords(self, api_client, therapist_user):
        ...
        assert response.data["error"]["details"]["new_password_confirm"] == ["As senhas não conferem."]
    ...
    def test_token_invalidation_after_password_reset(self, api_client, therapist_user):
        """
        Testa se tokens antigos são invalidados após reset de senha.
        """
        # 1. Autenticar o usuário
        login_url = reverse("auth-login")
        login_payload = {"email": therapist_user.email, "password": TEST_PASSWORD}
        ...
        reset_response = api_client.post(reset_url, reset_payload, format="json")
        ...
        new_login_response = api_client.post(
            login_url, {"email": therapist_user.email, "password": new_pass}, format="json"
        )
        ...
        print(f"Old Access Token: {me_response.status_code} (Valid? {access_valid_after_reset})")
        print(f"Old Refresh Token: {refresh_response.status_code} " f"(Valid? {refresh_valid_after_reset})")
        print("--------------------------------\n")
    ...
    def test_token_cannot_be_reused(self, api_client, therapist_user):
        ...
        assert res2.data["error"]["details"]["token"] == ["O link de redefinição é inválido ou expirou."]
