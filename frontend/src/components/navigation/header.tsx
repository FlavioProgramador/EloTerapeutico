...
import { membershipRoleLabels } from "@/features/organizations/types";
...
export function Header({ className }: HeaderProps) {
  ...
  const roleLabel = activeMembership
    ? membershipRoleLabels[activeMembership.role]
    : user?.role === "secretary"
      ? "Secretária"
      : user?.role === "admin"
        ? "Administrador"
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="hidden text-left sm:block">
              <p className="max-w-40 truncate text-xs font-bold leading-none text-foreground">
          ...
          {isProfileOpen && (
            <>
              ...
              <div className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-xl border border-border bg-popover shadow-xl shadow-black/10">
                ...
                <div className="p-1.5">
                  <Link
                    href="/dashboard/configuracoes/perfil-profissional"
                    onClick={() => setIsProfileOpen(false)}
                  ...
                  <Link
                    href="/dashboard/configuracoes/organizacao"
                    onClick={() => setIsProfileOpen(false)}
                    className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-xs text-muted-foreground transition hover:bg-secondary hover:text-foreground"
                  >
