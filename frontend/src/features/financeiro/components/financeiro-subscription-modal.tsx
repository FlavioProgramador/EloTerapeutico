...
interface Props { open: boolean; patients: Patient[]; onClose: () => void }
...
export function FinanceiroSubscriptionModal({ open, patients, onClose }: Props) {
  ...
  const form = useForm<SubscriptionFormData>({
    ...
    defaultValues: { frequency: "weekly", weekday: 0, appointment_time: "09:00", duration_minutes: 50, monthly_amount: "", due_day: 5, preferred_payment_method: "uninformed", reminder_days_before: 0, notes: "" },
  });

  useEffect(() => {
    if (open) form.reset({ patient: undefined, frequency: "weekly", weekday: 0, appointment_time: "09:00", first_appointment_date: "", duration_minutes: 50, monthly_amount: "", due_day: 5, first_due_date: "", reminder_days_before: 0, preferred_payment_method: "uninformed", payment_link: "", notes: "" });
  }, [open, form]);

  const submit = form.handleSubmit((data) => mutation.mutate({
    ...data,
  ...
  return (
    <Modal isOpen={open} onClose={onClose} title="Nova mensalidade" description="Configure uma assinatura mensal para o paciente." className="max-w-xl">
      <form className="space-y-5" onSubmit={submit}>
        <label className="block space-y-1.5 text-sm font-semibold">Paciente *
          <select className="h-11 w-full rounded-lg border border-input bg-card px-3" {...form.register("patient", { valueAsNumber: true })}>
        ...
        <div className="border-t border-border pt-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-1.5 text-sm font-semibold">Frequência
              <select className="h-11 w-full rounded-lg border border-input bg-card px-3" {...form.register("frequency")}><option value="weekly">Semanal</option><option value="biweekly">Quinzenal</option><option value="monthly">Mensal</option></select>
            </label>
            <label className="space-y-1.5 text-sm font-semibold">Dia da semana
              <select className="h-11 w-full rounded-lg border border-input bg-card px-3" {...form.register("weekday", { valueAsNumber: true })}>{weekdays.map((day, index) => <option key={day} value={index}>{day}</option>)}</select>
            </label>
            <Input label="Horário" type="time" {...form.register("appointment_time")} />
            <Input label="Duração (minutos)" type="number" min={15} {...form.register("duration_minutes", { valueAsNumber: true })} />
        ...
        <div className="border-t border-border pt-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Input label="Dia do vencimento" type="number" min={1} max={28} {...form.register("due_day", { valueAsNumber: true })} />
            <Input label="Vencimento da 1ª cobrança" type="date" {...form.register("first_due_date")} />
            <Input label="Lembrete (dias antes)" type="number" min={0} max={30} {...form.register("reminder_days_before", { valueAsNumber: true })} />
          </div>
        ...
        <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onClose}>Cancelar</Button><Button type="submit" isLoading={mutation.isPending}>Criar mensalidade</Button></div>
      </form>
    </Modal>
  );
}
