...
function range(preset: string) {
  ...
  if (preset === "next30") {
    ...
    return { startDate: now.toISOString().slice(0, 10), endDate: end.toISOString().slice(0, 10) };
  }
...
export function useFinanceiroPage() {
  ...
  useEffect(() => setHidden(localStorage.getItem("financeiro-hide-values") === "true"), []);
  const toggleHidden = () => setHidden((value) => {
  ...
  const transactions = useTransactions({ transaction_type: tab === "subscriptions" ? undefined : tab, payment_status: statusFilter === "all" ? undefined : statusFilter });
  ...
  const visibleTransactions = useMemo(() => (transactions.data ?? []).filter((item) => {
    ...
    const itemDate = item.due_date || item.payment_date || item.created_at.slice(0, 10);
