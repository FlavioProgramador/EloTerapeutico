    ...
    <main className="grid min-h-screen place-items-center bg-[#F7FAF8] px-5 py-12 text-[#1A2E26]">
      <section className="w-full max-w-2xl rounded-[32px] border border-[#1A2E26]/10 bg-white p-7 text-center shadow-sm sm:p-10">
        ...
        <div className="mt-7 rounded-2xl border border-gray-100 bg-gray-50 p-5 text-left text-sm">
          {loading ? (
            ...
            <dl className="grid gap-3 sm:grid-cols-2">
              <div><dt className="text-gray-500">Plano</dt><dd className="font-bold">{subscription.plan.name}</dd></div>
              <div><dt className="text-gray-500">Status</dt><dd className="font-bold">{subscription.status}</dd></div>
              {subscription.trial_ends_at && <div><dt className="text-gray-500">Fim do teste</dt><dd className="font-bold">{formatDate(subscription.trial_ends_at)}</dd></div>}
              {subscription.current_period_end && <div><dt className="text-gray-500">Fim do período</dt><dd className="font-bold">{formatDate(subscription.current_period_end)}</dd></div>}
            </dl>
          ) : (
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          ...
          <Link href="/dashboard/assinatura" className="rounded-xl border border-gray-200 px-5 py-3 text-sm font-bold hover:bg-gray-50">Gerenciar assinatura</Link>
          <Link href="/" className="rounded-xl border border-gray-200 px-5 py-3 text-sm font-bold hover:bg-gray-50">Página inicial</Link>
        </div>
      </section>
    </main>
  );
