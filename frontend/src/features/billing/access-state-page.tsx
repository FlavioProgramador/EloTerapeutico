...
const copy = {
  pending: {
    ...
    description:
      "A cobrança foi criada, mas o acesso só será liberado após a confirmação segura recebida pelo webhook do gateway.",
    icon: Clock3,
    primary: {
      href: "/dashboard/assinatura/faturas",
      label: "Acompanhar faturas",
    },
  },
  "past-due": {
    ...
    description:
      "Entre na área de faturas para atualizar a cobrança ou escolher uma nova forma de pagamento. Seus dados permanecem preservados.",
    icon: AlertTriangle,
    primary: {
      href: "/dashboard/assinatura/faturas",
      label: "Regularizar cobrança",
    },
  },
  expired: {
    ...
    description:
      "O teste ou período contratado terminou. Sua conta e seus dados continuam salvos, mas as ferramentas internas estão bloqueadas.",
    icon: RefreshCw,
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#F7FAF8] px-5 py-12 text-[#1A2E26]">
      <section className="w-full max-w-2xl rounded-[32px] border border-[#1A2E26]/10 bg-white p-7 text-center shadow-sm sm:p-10">
        <div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-[#FFF7ED] text-[#F97316]">
          <Icon className="h-8 w-8" />
        </div>
        <p className="mt-6 text-xs font-extrabold uppercase tracking-[0.22em] text-[#F97316]">
          {content.eyebrow}
        </p>
          ...
          {loading ? (
            ...
            <dl className="grid gap-3 sm:grid-cols-2">
              ...
              {subscription.trial_ends_at && (
                <div>
                  <dt className="text-gray-500">Fim do teste</dt>
                  <dd className="font-bold text-gray-900">
              ...
              {subscription.current_period_end && (
                <div>
                  <dt className="text-gray-500">Fim do período</dt>
                  <dd className="font-bold text-gray-900">
            ...
            <p className="text-gray-600">
              Nenhuma assinatura operacional foi localizada. Escolha um plano
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#F97316] px-5 py-3 text-sm font-bold text-white hover:bg-[#EA580C]"
          >
            <CreditCard className="h-4 w-4" /> {content.primary.label}
          </Link>
          <Link
