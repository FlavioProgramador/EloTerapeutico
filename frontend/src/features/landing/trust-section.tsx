    ...
    <section id="seguranca" className="trust-section">
      <div className="trust-section__inner">
        <Reveal className="trust-section__visual">
          ...
          <span className="trust-node trust-node--one"><KeyRound /></span>
          <span className="trust-node trust-node--two"><UsersRound /></span>
          <span className="trust-node trust-node--three"><Activity /></span>
        </Reveal>
        ...
        <Reveal className="trust-section__copy" delay={0.08}>
          ...
          <p>
            O Elo Terapêutico adota controles técnicos para reduzir exposição indevida
            e separar responsabilidades. A conformidade completa também depende da
            configuração, da operação e dos processos da clínica.
          </p>

          <div className="trust-points">
            {points.map((point) => (
              <article key={point.title}>
                <span><point.icon aria-hidden="true" /></span>
                <div><strong>{point.title}</strong><small>{point.text}</small></div>
              </article>
            ))}
          </div>
        </Reveal>
