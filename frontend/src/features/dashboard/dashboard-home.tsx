...
function Metric({ title, value, note, icon: Icon, tone = "primary" }: {
  ...
  return (
    <motion.div variants={metricVariants} whileHover={{ y: -4 }} className="transition-all duration-200">
...
function Bars({ values, labels, barClassName = "bg-primary/75" }: { values: number[]; labels: string[]; barClassName?: string }) {
  ...
  return (
    <div className="mt-5 flex h-52 items-end gap-3 border-b border-l border-border px-3 pb-7">
      {values.map((value, index) => {
        ...
        return (
          <div key={`${labels[index]}-${index}`} className="relative flex h-full flex-1 items-end justify-center">
            <motion.div
              initial={{ height: 0 }}
              animate={{ height: `${heightPercent}%` }}
              transition={{ duration: 0.6, ease: "easeOut", delay: index * 0.05 }}
              className={cn("w-7 rounded-t hover:brightness-110 cursor-pointer transition-all", barClassName)}
              title={String(value)}
            />
            <span className="absolute -bottom-6 text-[10px] text-muted-foreground">{labels[index]}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => setShowValues((value) => !value)} leftIcon={showValues ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}>
        ...
        <motion.div variants={sectionVariants}>
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader className="flex flex-row items-start justify-between pb-3">
              <div>
                <CardTitle className="text-base">Configure sua clínica</CardTitle>
                <p className="mt-1 text-xs text-muted-foreground">{setupDone} de {setup.length} passos concluídos</p>
              </div>
              <button type="button" onClick={hideSetup} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
                <X className="h-4 w-4" /> Dispensar
              </button>
            </CardHeader>
            <CardContent>
              <div className="mb-4 h-1.5 overflow-hidden rounded-full bg-secondary">
                <div className="h-full rounded-full bg-primary" style={{ width: `${(setupDone / setup.length) * 100}%` }} />
              </div>
              <div className="divide-y divide-border">
                {setup.map((item) => (
                  <button type="button" key={item.title} disabled={!item.action} onClick={() => item.action && router.push(item.action)} className="flex w-full items-center gap-3 py-3 text-left disabled:cursor-default">
                    <span className={cn("h-4 w-4 rounded-full border", item.done ? "border-success bg-success" : "border-border-strong")} />
      ...
      <motion.section variants={containerVariants} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <motion.section variants={sectionVariants} className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">Receitas pagas nos últimos 6 meses</p>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">Atendimentos concluídos neste mês</p>
          </CardHeader>
          <CardContent>
            <Bars values={weekdayCounts} labels={["Seg", "Ter", "Qua", "Qui", "Sex", "Sáb", "Dom"]} barClassName="bg-success/75" />
          </CardContent>
        </Card>
      </motion.section>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Agenda do dia</p>
            </div>
            <Button size="sm" variant="outline" onClick={() => router.push("/dashboard/agenda")}>Ver agenda</Button>
          </CardHeader>
          <CardContent>
            {(today.data ?? []).length === 0 ? (
              <p className="py-10 text-center text-sm text-muted-foreground">Nenhuma consulta hoje.</p>
            ) : (
              <div className="divide-y divide-border">
                {(today.data ?? []).slice(0, 6).map((item) => (
                  <button type="button" key={item.id} onClick={() => router.push(`/dashboard/agenda?appointment=${item.id}`)} className="flex w-full items-center gap-3 py-3 text-left hover:bg-secondary/40 transition-colors">
                    <strong className="w-12 text-xs">{new Date(item.start_time).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</strong>
                    <span className="flex-1 text-xs font-semibold">{item.patient_name || "Paciente"}</span>
                    <span className="text-[10px] text-muted-foreground">{item.status_display || item.status}</span>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between text-base">
              Alertas <span className="rounded-full border px-2 py-0.5 text-xs text-muted-foreground">{overdue.length}</span>
            </CardTitle>
            <p className="text-xs text-muted-foreground">Cobranças vencidas</p>
          </CardHeader>
          <CardContent>
            {overdue.length === 0 ? (
              <p className="py-10 text-center text-sm text-muted-foreground">Nenhum alerta pendente.</p>
            ) : (
              <div className="divide-y divide-border">
                {overdue.slice(0, 6).map((item) => (
                  <button type="button" key={item.id} onClick={() => router.push(`/dashboard/financeiro?transaction=${item.id}`)} className="flex w-full items-center gap-3 py-3 text-left hover:bg-secondary/40 transition-colors">
                    <AlertTriangle className="h-4 w-4 text-warning" />
                    <span className="flex-1">
                      <strong className="block text-xs">{item.patient_name || item.description}</strong>
                      <span className="block text-[10px] text-muted-foreground">Venceu em {new Date(`${item.due_date}T00:00:00`).toLocaleDateString("pt-BR")}</span>
                    </span>
                    <strong className="text-xs">{displayMoney(numberValue(item.amount))}</strong>
                  </button>
                ))}
              </div>
            )}
