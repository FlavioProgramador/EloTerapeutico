...
export const communicationsService = {
  async dashboard(period = "30") { const end = new Date(); const start = new Date(end); start.setDate(start.getDate() - Number(period)); const { data } = await api.get<CommunicationDashboard>("communications/dashboard/", { params: { start_date: start.toISOString(), end_date: end.toISOString() } }); return data; },
  async list(filters?: Record<string, string | number | undefined>) { const { data } = await api.get<Paginated<Communication>>("communications/", { params: params(filters) }); return data; },
  async detail(id: string) { const { data } = await api.get<CommunicationDetail>(`communications/${id}/`); return data; },
  async create(payload: CreateCommunicationPayload) { const { data } = await api.post<CommunicationDetail>("communications/", payload, { headers: { "Idempotency-Key": crypto.randomUUID() } }); return data; },
  async action(id: string, action: "cancel" | "retry" | "send" | "mark-manually-sent" | "open-manual") { const { data } = await api.post(action === "open-manual" ? `communications/${id}/open-manual/` : `communications/${id}/${action}/`); return data; },
  async templates(filters?: Record<string, string | undefined>) { const { data } = await api.get<Paginated<CommunicationTemplate> | CommunicationTemplate[]>("communications/templates/", { params: params(filters) }); return Array.isArray(data) ? data : data.results; },
  ...
  async markNotificationRead(id: number) { const { data } = await api.post<InAppNotification>(`communications/notifications/${id}/read/`); return data; },
  async readAllNotifications() { const { data } = await api.post<{ updated: number }>("communications/notifications/read-all/"); return data; },
};
