...
import { communicationChannelLabel, communicationConnectionStatusLabel } from "./communications.utils";
import { useRemoveCommunicationChannel, useSendCommunicationChannelTest, useTestCommunicationChannelConnection, useToggleCommunicationChannel, useUpdateCommunicationChannel } from "./use-communications";
import type { ChannelConfigurationField, CommunicationChannelConfig, UpdateChannelConfigurationPayload } from "./types";

const steps = ["Provedor", "Credenciais", "Remetente", "Preferências", "Teste", "Ativação"];
const senderFields = new Set(["sender", "sender_name", "sender_email", "reply_to", "phone_number"]);
const preferenceFields = new Set(["signature", "tracking_enabled", "desktop_enabled", "sound_enabled", "duration_seconds", "open_in_web", "country_code", "timeout", "monthly_limit", "default_language", "api_version"]);

function humanError(error: unknown): string {
  const response = (error as { response?: { data?: unknown } })?.response?.data;
  if (typeof response === "string") return response;
  if (response && typeof response === "object") {
    const values = Object.values(response as Record<string, unknown>).flatMap((value) => Array.isArray(value) ? value : [value]);
...
export function ChannelConfigurationModal({ channel, onClose }: { channel: CommunicationChannelConfig; onClose: () => void }) {
  ...
  function updateField(field: ChannelConfigurationField, value: string | number | boolean) {
    if (field.secret) setSecrets((current) => ({ ...current, [field.name]: String(value) }));
  ...
  async function save(saveAsDraft: boolean, showToast = true) {
    ...
    const providerChanged = Boolean(currentChannel.provider && currentChannel.provider !== providerId);
    const confirmed = !providerChanged || !currentChannel.is_active || window.confirm("Trocar o provedor desativará o canal atual. Deseja continuar?");
    if (!confirmed) return null;
    const payload: UpdateChannelConfigurationPayload = {
      provider: providerId,
      metadata,
      secrets: Object.fromEntries(Object.entries(secrets).filter(([, value]) => String(value).trim() !== "")),
      sender: String(metadata.sender_email || metadata.sender || ""),
      public_identifier: String(metadata.phone_number_id || metadata.account_sid || ""),
      save_as_draft: saveAsDraft,
      confirm_provider_change: providerChanged && currentChannel.is_active,
    };
    try {
      const result = await updateChannel.mutateAsync({ channel: currentChannel.channel, payload });
      setCurrentChannel(result);
      if (showToast) toast.success(saveAsDraft ? "Configuração salva como rascunho." : "Configuração salva. Agora valide a conexão.");
      return result;
    } catch (error) {
  ...
  async function handleSendTest() {
    ...
    if (!saved) return;
    try {
      const result = await sendTest.mutateAsync({ channel: currentChannel.channel, destination: testDestination || undefined });
      setCurrentChannel(result.channel);
      const manualUrl = result.test.metadata.manual_url;
      setManualTestUrl(manualUrl || null);
      toast.success(manualUrl ? "Link de teste preparado. O envio continua manual." : "Mensagem de teste processada.");
    } catch (error) {
      toast.error(humanError(error));
    }
  }
  ...
  async function handleToggle() {
    try {
      const result = await toggleChannel.mutateAsync({ channel: currentChannel.channel, active: !currentChannel.is_active });
      setCurrentChannel(result);
      toast.success(currentChannel.is_active ? "Canal desativado." : "Canal ativado.");
    } catch (error) {
      toast.error(humanError(error));
    }
  }
  ...
  async function handleRemove() {
    if (!window.confirm("Remover esta configuração? As credenciais armazenadas serão apagadas.")) return;
    try {
  ...
  function renderField(field: ChannelConfigurationField) {
    ...
    return <label key={field.name} className="grid gap-2 text-xs font-semibold text-foreground">
      <span>{field.label}{field.required && <span className="ml-1 text-danger">*</span>}</span>
      {field.kind === "boolean" ? <span className="flex h-11 items-center gap-3 rounded-xl border border-input bg-background px-3"><input type="checkbox" checked={Boolean(currentValue)} disabled={field.read_only || pending} onChange={(event) => updateField(field, event.target.checked)} /><span className="font-normal text-muted-foreground">{Boolean(currentValue) ? "Ativado" : "Desativado"}</span></span> : field.kind === "textarea" ? <textarea className="min-h-28 rounded-xl border border-input bg-background p-3 text-sm font-normal outline-none focus:border-primary" value={String(currentValue)} disabled={field.read_only || pending} placeholder={field.placeholder} onChange={(event) => updateField(field, event.target.value)} /> : <input className={commonClass} type={field.kind === "number" ? "number" : field.kind === "password" ? "password" : field.kind === "email" ? "email" : field.kind === "tel" ? "tel" : field.kind === "url" ? "url" : "text"} value={String(currentValue)} disabled={field.read_only || pending} placeholder={configuredSecret ? "Segredo já configurado — deixe vazio para manter" : field.placeholder} onChange={(event) => updateField(field, field.kind === "number" ? Number(event.target.value) : event.target.value)} />}
      {configuredSecret && <span className="text-[10px] font-normal text-success">Credencial armazenada de forma criptografada.</span>}
    </label>;
  }
