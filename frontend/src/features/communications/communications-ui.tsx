...
export function statusTone(status: CommunicationStatus) {
  if (["delivered", "read", "responded"].includes(status))
    return "border-success/20 bg-success/10 text-success";
  if (["failed", "expired"].includes(status))
    return "border-danger/20 bg-danger/10 text-danger";
  if (["scheduled", "queued", "processing"].includes(status))
    return "border-warning/20 bg-warning/10 text-warning";
  return "border-border bg-secondary text-muted-foreground";
}
export function channelIcon(channel: CommunicationChannel) {
  if (channel === "email") return Mail;
  if (channel === "in_app") return Bell;
  if (channel.startsWith("whatsapp")) return MessageCircle;
  return Smartphone;
}
