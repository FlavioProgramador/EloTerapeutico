...
export function CommunicationsPage() {
  ...
  return <main className="min-h-full bg-background p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl">
    ...
    <nav className="mt-7 flex gap-1 overflow-x-auto rounded-xl border border-border bg-card p-1" aria-label="Seções de comunicações">{tabs.map(([value, label]) => <button key={value} type="button" onClick={() => setTab(value)} className={cn("whitespace-nowrap rounded-lg px-4 py-2 text-xs font-semibold transition", tab === value ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>{label}</button>)}</nav>

    {tab === "overview" && <section className="mt-6 grid gap-6"><div className="flex justify-end"><select value={period} onChange={(event) => setPeriod(event.target.value)} className="h-10 rounded-xl border border-input bg-card px-3 text-xs"><option value="7">Esta semana</option><option value="30">Últimos 30 dias</option><option value="90">Últimos 90 dias</option></select></div>
      {dashboard.isLoading && <div className="flex min-h-56 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Carregando indicadores reais...</div>}
      {dashboard.isError && <div className="rounded-xl border border-danger/20 bg-danger/5 p-5 text-sm text-danger">Não foi possível carregar os indicadores.</div>}
      {dashboard.data && <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><Metric title="Enviadas no período" value={dashboard.data.metrics.total} icon={Send} detail="Registros reais criados" /><Metric title="Agendadas" value={dashboard.data.metrics.scheduled} icon={CalendarClock} detail="Aguardando o horário" /><Metric title="Entregues" value={dashboard.data.metrics.delivered} icon={CheckCircle2} detail="Inclui lidas e respondidas" /><Metric title="Falhas" value={dashboard.data.metrics.failed} icon={AlertTriangle} detail="Erros sanitizados" /><Metric title="Taxa de sucesso" value={`${dashboard.data.metrics.success_rate}%`} icon={ShieldCheck} detail="Enviadas ou entregues" /></div><div className="grid gap-4 lg:grid-cols-2"><Card><CardContent className="p-5"><h2 className="text-sm font-bold">Comunicações por canal</h2><div className="mt-5 grid gap-4">{dashboard.data.by_channel.length === 0 && <p className="text-xs text-muted-foreground">Nenhuma comunicação registrada neste período.</p>}{dashboard.data.by_channel.map((item) => <div key={item.channel}><div className="mb-1 flex justify-between text-xs"><span>{channelLabel[item.channel]}</span><b>{item.total}</b></div><div className="h-2 rounded-full bg-secondary"><div className="h-2 rounded-full bg-primary" style={{ width: `${Math.max(6, (item.total / maxChannel) * 100)}%` }} /></div></div>)}</div></CardContent></Card><Card><CardContent className="p-5"><h2 className="text-sm font-bold">Status da operação</h2><div className="mt-5 flex flex-wrap gap-2">{dashboard.data.by_status.length === 0 && <p className="text-xs text-muted-foreground">Nenhuma comunicação registrada neste período.</p>}{dashboard.data.by_status.map((item) => <span key={item.status} className={cn("rounded-full border px-3 py-1.5 text-xs font-bold", statusTone(item.status))}>{statusLabel[item.status]}: {item.total}</span>)}</div></CardContent></Card></div></>}
    </section>}

    {tab === "history" && <section className="mt-6"><div className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-[1fr_180px_180px]"><label className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar paciente, assunto ou origem..." className="h-10 w-full rounded-lg border border-input bg-background pl-10 pr-3 text-xs" /></label><select value={channel} onChange={(event) => setChannel(event.target.value)} className="h-10 rounded-lg border border-input bg-background px-3 text-xs"><option value="">Todos os canais</option>{Object.entries(channelLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 rounded-lg border border-input bg-background px-3 text-xs"><option value="">Todos os status</option>{Object.entries(statusLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><div className="mt-4 overflow-hidden rounded-xl border border-border bg-card">{communications.isLoading && <div className="flex min-h-64 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Carregando histórico...</div>}{communications.isError && <div className="p-6 text-sm text-danger">Não foi possível carregar o histórico.</div>}{communications.data?.results.length === 0 && <div className="grid min-h-64 place-items-center"><EmptyState icon={<MessageSquare className="h-6 w-6" />} title="Nenhuma comunicação encontrada" description="As mensagens, lembretes e notificações aparecerão aqui." /></div>}{!!communications.data?.results.length && <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs"><thead className="border-b border-border bg-secondary/40 text-muted-foreground"><tr><th className="px-4 py-3">Paciente</th><th className="px-4 py-3">Canal</th><th className="px-4 py-3">Assunto</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Data</th><th className="px-4 py-3">Ação</th></tr></thead><tbody>{communications.data.results.map((item) => { const Icon = channelIcon(item.channel); return <tr key={item.public_id} className="border-b border-border/70 last:border-0"><td className="px-4 py-4 font-semibold">{item.patient_name || "Sistema"}<p className="mt-1 text-[10px] font-normal text-muted-foreground">{item.recipient}</p></td><td className="px-4 py-4"><span className="inline-flex items-center gap-2"><Icon className="h-4 w-4 text-primary" />{channelLabel[item.channel]}</span></td><td className="max-w-xs truncate px-4 py-4">{item.subject || "Sem assunto"}</td><td className="px-4 py-4"><span className={cn("rounded-full border px-2.5 py-1 text-[10px] font-bold", statusTone(item.status))}>{statusLabel[item.status]}</span></td><td className="px-4 py-4 text-muted-foreground">{formatDate(item.scheduled_at || item.created_at)}</td><td className="px-4 py-4"><button type="button" onClick={() => setSelectedId(item.public_id)} className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 font-semibold text-primary hover:bg-primary/10"><Eye className="h-3.5 w-3.5" />Detalhes</button></td></tr>; })}</tbody></table></div>}</div></section>}

    {tab === "templates" && <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{templates.isLoading && <p className="text-sm text-muted-foreground">Carregando templates...</p>}{templates.data?.map((template) => <Card key={template.id}><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-primary/10 text-primary"><FileText className="h-5 w-5" /></span><span className="rounded-full border border-border px-2 py-1 text-[10px]">{template.is_system_template ? "Sistema" : "Personalizado"}</span></div><h2 className="mt-4 text-sm font-bold">{template.name}</h2><p className="mt-2 line-clamp-3 text-xs text-muted-foreground">{template.body_template}</p><div className="mt-4 flex flex-wrap gap-1">{template.allowed_variables.slice(0, 4).map((variable) => <span key={variable} className="rounded bg-secondary px-2 py-1 text-[9px] text-muted-foreground">{`{{${variable}}}`}</span>)}</div></CardContent></Card>)}</section>}
