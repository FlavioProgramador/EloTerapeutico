...
export const recordsService = {
  ...
  list: async (filters?: RecordFilters): Promise<EvolutionListItem[]> => {
    ...
    const response = await api.get<EvolutionListItem[] | { results: EvolutionListItem[] }>(
  ...
  create: async (data: CreateEvolutionPayload): Promise<EvolutionDetail> => {
    const response = await api.post<EvolutionDetail>("records/evolutions/", data);
    return response.data;
  },
  ...
  update: async (
    ...
    const response = await api.patch<EvolutionDetail>(`records/evolutions/${id}/`, data);
    return response.data;
  },
   ...
   */
  saveAnamnesis: async (patientId: number, data: Anamnesis, exists: boolean): Promise<Anamnesis> => {
    if (exists) {
      const response = await api.put<Anamnesis>(`records/patients/${patientId}/anamnesis/`, data);
      return response.data;
    } else {
      const response = await api.post<Anamnesis>(`records/patients/${patientId}/anamnesis/`, data);
      return response.data;
    }
  },
  ...
  createAddendum: async (
    ...
    const response = await api.post<Addendum>(`records/evolutions/${evolutionId}/addendum/`, data);
    return response.data;
  },
};
